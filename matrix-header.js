...
function blink() {
    ...
    let randomness =5; // how often chars will blink
    const outerTimer = setInterval(()=>{ // outer timer is a var if later blinking would be dicountinued
        let r = Math.ceil(Math.random()*randomness);
        if (r === randomness) {              
            let randomTd = selectedTds[Math.floor(Math.random()*selectedTds.length)];
            randomTd.style.color = "rgba(50, 50, 50, 0.3)";
            setTimeout(()=>{randomTd.style.color = "rgb(165, 197, 222)"},20); // set original color back
        } // end of if
